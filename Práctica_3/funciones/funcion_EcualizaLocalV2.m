function Ieq_local = funcion_EcualizaLocalV2(I, NumFilVent, NumColVent,OpcionRelleno)
%FUNCION_ECUALIZALOCAL Función para ecualizar de forma local, a nivel de
%ventana 5x5. Donde I es la imagen a ecualizar y opción de
...
%             Ia = [zona_izquierda Ia zona_derecha];
    ...
    Ieq_local = double(zeros(M,N));
    
    for i=1+aumento_fil:tamVentana:M+aumento_fil
        for j=1+aumento_col:tamVentana:N+aumento_col
            ...
            Ieq_local(i-aumento_fil:i-aumento_fil+tamVentana-1,j-aumento_col:j-aumento_col+tamVentana-1)...
                = Ieq(centroM-2:centroM+2,centroN-2:centroN+2);
...
%aplicarla para generar la imagen de salida.
    ...
    [M,N] = size(I);
    L = size(unique(I),1); % Número de niveles de grises
